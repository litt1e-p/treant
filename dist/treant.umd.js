(function (global, factory) {
	...
	(global = global || self, factory(global.treant = {}));
	...
	var Raphael = unwrapExports(raphael_min);
	var raphael_min_1 = raphael_min.Raphael;
	...
	require('perfect-scrollbar'); // ;(function( exports ){


	var UTIL = {
	  inheritAttrs: function inheritAttrs(me, from) {
	...
	*/
	...
	var TreeStore = {
	  ...
	  destroy: function destroy(tree_id) {
	  ...
	  */
	  ...
	  positionNodes: function positionNodes() {
	    ...
	    negOffsetX = treeSize.x.min + deltaX <= 0 ? Math.abs(treeSize.x.min) : 0,
	        ...
	        len,
	        node;
	    this.handleOverflow(treeWidth, treeHeight); // position all the nodes

	    for (i = 0, len = this.nodeDB.db.length; i < len; i++) {
	      node = this.nodeDB.get(i);
	      if (node.id === 0 && this.CONFIG.hideRootNode) continue; // if the tree is smaller than the draw area, then center the tree within drawing area
	  ...
	  setConnectionToParent: function setConnectionToParent(node, hidePoint) {
	    ...
	    if (this.connectionStore[node.id]) {
	      ...
	      this.connectionStore[node.id] = connLine; // don't show connector arrows por pseudo nodes
	  ...
	  getPointPathString: function getPointPathString(hp) {
	...
	*/
	...
	var NodeDB = function NodeDB(nodeStructure, tree) {
	  ...
	  function itterateChildren(node, parentId) {
	    ...
	    if (node.children) {
	      newNode.children = []; // pseudo node is used for descending children to the next level
	      ...
	      var stack = node.stackChildren && !self.hasGrandChildren(node) ? newNode.id : null; // svildren are position on separate leves, one beneeth the other
	...
	*/
	...
	var TreeNode = function TreeNode(nodeStructure, id, parentId, tree, stackParentId) {
	  ...
	  this.prelim = 0;
	  this.modifier = 0;
	  this.stackParentId = stackParentId; // pseudo node is a node with width=height=0, it is invisible, but necessary for the correct positiong of the tree
	...
	TreeNode.prototype = {
	  ...
	  childrenCenter: function childrenCenter(tree) {
	  ...
	  addSwitchEvent: function addSwitchEvent(my_switch) {
	    var self = this;
	  ...
	  toggleCollapse: function toggleCollapse() {
	    ...
	    if (!tree.inAnimation) {
	      ...
	      if (this.collapsed) {
	        jquery(this.nodeDOM).addClass('collapsed');
	      } else {
	        jquery(this.nodeDOM).removeClass('collapsed');
	      }
	  ...
	  hide: function hide(collapse_to_point) {
	    ...
	    var jq_node = jquery(this.nodeDOM),
	        tree = this.Tree(),
	        config = tree.CONFIG,
	        new_pos = {
	  ...
	  show: function show() {
	    ...
	    var new_pos = {
	      ...
	      top: this.Y
	        ...
	        tree = this.Tree(),
	        config = tree.CONFIG; // if the node was hidden, update width and height
	    ...
	    jquery(this.nodeDOM).animate(new_pos, config.animation.nodeSpeed, config.animation.nodeAnimation, function () {
	      ...
	      if (this.text) {
	        for (var key in this.text) {
	          if (TreeNode.CONFIG.textClass[key]) {
	            var text = document.createElement(this.text[key].href ? 'a' : 'p'); // meke an <a> element if required
	...
	*/
	...
	var Treant = function Treant(jsonConfig, callback) {
